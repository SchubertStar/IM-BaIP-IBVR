...
def longest_diagonal(points_2d):
    max_distance = 0
    best_pair = None
    # print('points_2d: %s'%points_2d)
    # Compute distance for all pairs of 8 points
    for (p1, p2) in itertools.combinations(points_2d, 2):
        d = np.linalg.norm(np.array(p1) - np.array(p2))  # Euclidean distance
        if d > max_distance:
            max_distance = d
            best_pair = (p1, p2)

    return max_distance, best_pair

def calc_theta_ray(img, box_2d, proj_matrix):
    height, width = img.shape[:2]
    fovx = 2 * np.arctan(width / (2 * proj_matrix[0][0]))
    fovy = 2 * np.arctan(height / (2 * proj_matrix[1][1]))

    # center = (box_2d[1][0] + box_2d[0][0]) / 2
    center_x = (box_2d[1][0] + box_2d[0][0]) / 2
    center_y = (box_2d[1][1] + box_2d[0][1]) / 2
    # print('center_x: %s'%center_x)
    # print('center_y: %s'%center_y)

    # Compute horizontal angle (theta_x)
    dx = center_x - (width / 2)
    mult_x = -1 if dx < 0 else 1
    dx = abs(dx)
    theta_x = np.arctan((2 * dx * np.tan(fovx / 2)) / width) * mult_x

    # Compute vertical angle (theta_y)
    dy = center_y - (height / 2)
    mult_y = -1 if dy < 0 else 1
    dy = abs(dy)
    theta_y = np.arctan((2 * dy * np.tan(fovy / 2)) / height) * mult_y

    return center_x, center_y, theta_x, theta_y 
...
def create_corners(dimension, location=None, R=None):
    ...
    # print('final_corners: %s'%final_corners)
